...
const updateClock = async(cid, clock) =>{
    try {
        const updatedClock = await db.one("UPDATE clocks SET name=$1, description=$2, image=$3, dimensions=$4, color=$5, material=$6, price=$7, rating=$8, featured=$9, quantity=$10 WHERE cid=$11 RETURNING *",
        [clock.name, clock.description, clock.image, clock.dimensions, clock.color, clock.material, clock.price, clock.rating, clock.featured, clock.quantity, cid]);
        return updatedClock;
    } catch (error) {
        return error;
    }
